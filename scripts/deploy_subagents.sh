...
ENV="${3:-${ENV:-staging}}"
GATEWAY_NAME="${4:-${GATEWAY_NAME:-version1-gateway}}"

if [[ -z "${PROJECT_ID}" ]]; then
  echo "Usage: $0 <PROJECT_ID> [REGION] [ENV] [GATEWAY_NAME]"
  echo "Example: $0 my-staging-proj asia-northeast3 staging version1-gateway"
  exit 1
fi

SUBAGENT_SA="version1-subagent@${PROJECT_ID}.iam.gserviceaccount.com"
GATEWAY_URI="projects/${PROJECT_ID}/locations/${REGION}/agentGateways/${GATEWAY_NAME}"
...
echo "Agent Gateway:    ${GATEWAY_URI}"
echo "============================================================"
...
for agent in "${SUBAGENTS[@]}"; do
  ...
  if [[ "${ENABLE_AGENT_GATEWAY:-false}" == "true" && -n "${GATEWAY_URI:-}" ]]; then
    echo "    Attaching Agent Gateway: ${GATEWAY_URI}"
    DEPLOY_FLAGS+=(--agent-gateway-ingress="${GATEWAY_URI}")
  fi
  (
    cd "app/agents/${agent}"
