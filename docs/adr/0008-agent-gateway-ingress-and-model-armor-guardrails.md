...
## Context
Marketing Value Creator (MVC) deploys four domain-specialized subagents ([P1] Market Sensing, [P2] Strategy & Brief, [P3] Creative Content, [P4] Performance & Insights) to Agent Platform Agent Runtime (Reasoning Engine) in `asia-northeast3` (Seoul). The central Cloud Run Orchestrator coordinates campaign execution by invoking these subagents via the Agent-to-Agent (A2A) protocol.
