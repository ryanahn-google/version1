...
- **Evaluation Cadence**: Automated on PRs (`pr-version1`), pre-deployment Staging verification, and on-demand via `agents-cli eval generate` and `agents-cli eval grade`.
...
| :--- | :--- | :---: | :--- |
| **Master Golden Campaigns (9 Scenarios)** | `tests/eval/datasets/golden_campaigns.json` | E2E DAG (Orchestrator + P1-P4) | 4 Flagship campaigns (Neo QLED 8K, Bespoke AI, NovaBuds Pro, Odyssey Ark), 3 Edge cases (micro-budget, flash sale, bilingual edit), 2 Guardrail probes |
...
- **44% Hand-Crafted Flagship Scenarios (4)**: Neo QLED 8K (TV), Bespoke AI (Laundry), NovaBuds Pro (Audio), Odyssey Ark G9 (Gaming Monitor).
- **33% Edge-Case Stress Scenarios (3)**: 48-hour flash sale, $5,000 hyper-local micro-budget, bilingual Korean campaign with marketer headline edit.
- **22% Hard Negatives & Guardrail Probes (2)**: Prompt injection / system override probe, competitor defamation / false claim probe (intercepted by Model Armor with 400 Bad Request).
...
- **Judge Model**: `gemini-3.1-pro-preview` / `gemini-3.1-pro` (Agent Platform `location="global"`, configured as `gemini-3.1-pro-preview` in `tests/eval/e2e_campaign_evaluator.py:48` with fallback resilience for `gemini-2.5-pro` and `gemini-2.5-flash`).
...
Each subagent can be evaluated directly on Agent Platform:
```bash
cd app/agents/market_sensing
agents-cli eval generate --dataset eval/datasets/golden-dataset.json
agents-cli eval grade --config eval/eval_config.yaml
```

### 5.2 Synthetic Marketer E2E Evaluator
Simulates an automated marketer driving the 4-stage DAG across all 9 scenarios:
```bash
