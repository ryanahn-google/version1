...
flowchart TB
    ...
    subgraph Staging_Env["Staging Project: capstone-staging-506811"]
        ...
        AGW_Staging["Agent Gateway (CLIENT_TO_AGENT)<br>version1-gateway"]
        ...
        AGW_Staging -.->|CONTENT_AUTHZ Authz Policy| Armor_Staging
    end
    ...
    CR_Staging <-->|A2A Ingress via Agent Gateway| AGW_Staging
    AGW_Staging <--> P1 & P2 & P3 & P4
...
- **Agent Gateway Ingress & Model Armor Content Authorization**: Subagents are fronted by a Google-managed Agent Gateway (`version1-gateway`) in `asia-northeast3` operating in `CLIENT_TO_AGENT` mode. Inbound prompts and JSON deliverables pass through inline Model Armor inspection via a Service Extensions Authz Extension (`CONTENT_AUTHZ`, fail-closed with `fail_open = false`). Subagents are deployed with SPIFFE-based Agent Identity (`--enable-agent-identity`) and bound via `--agent-gateway-ingress`, orchestrated by `scripts/deploy_subagents.sh` in the CI/CD pipeline.
