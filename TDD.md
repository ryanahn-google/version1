...
- **Dedicated Subagent Identity**: Deployed with `--service-account=version1-subagent@{project_id}.iam.gserviceaccount.com` (`service_accounts.tf`) bound to `roles/storage.objectAdmin`, `roles/aiplatform.user`, and `roles/serviceusage.serviceUsageConsumer`, enabling subagents on Agent Runtime to write visual deliverables directly to GCS while complying with least-privilege IAM.
...
- **Identity & Access Management (IAM)**:
  ...
  - Dedicated Subagent Service Account: `version1-subagent@{project_id}.iam.gserviceaccount.com` (roles: Vertex AI User, Storage Object Admin, Logging LogWriter, Cloud Trace Agent, Service Usage Consumer).
